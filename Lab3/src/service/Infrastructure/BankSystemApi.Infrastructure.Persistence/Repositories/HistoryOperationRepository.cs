...
public sealed class HistoryOperationRepository : IHistoryOperationRepository
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        RETURNING id, account_id, occurred_at, payload
        """;
    ...
    public async IAsyncEnumerable<HistoryOperation> QueryAsync(
        OperationHistoryQuery historyQuery,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        const string sql = """
        ...
        await using IPersistenceCommand command = connection.CreateCommand(sql)
            .AddParameter("accountIds", historyQuery.AccountIds.Select(i => i.Value))
            .AddParameter("page_size", historyQuery.PageSize)
            .AddParameter("cursor", historyQuery.IdCursor?.Value);
    ...
    private HistoryOperation CreateHistoryOperation(DbDataReader reader)
    {
        ...
        var historyEntry = new HistoryOperationEntry(
            ...
            new AccountId(reader.GetGuid("account_id")),

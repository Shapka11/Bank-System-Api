...
public sealed class InvoiceController : ControllerBase
{
    ...
    [EndpointDescription("Cancels an invoice. Only allowed for invoices with 'Approved/Created' status.")]
    ...
    [HttpPost("approve")]
    [AuthorizeFeature(Scope)]
    [EndpointName("ApproveInvoice")]
    [EndpointSummary("Approve an invoice")]
    [EndpointDescription("Approve an invoice. Only allowed for invoices with 'Created' status.")]
    [ProducesResponseType<ApproveInvoiceHttpResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status400BadRequest, MediaTypeNames.Application.ProblemJson)]
    public async Task<ActionResult<ApproveInvoiceHttpResponse>> Approve(
        [FromBody] ApproveInvoiceHttpRequest httpRequest,
        CancellationToken cancellationToken)
    {
        string userId = User.GetUserId();

        ApproveInvoiceResponse response = await _invoiceService.ApproveAsync(
            httpRequest.MapToApplication(userId),
            cancellationToken);

        return response switch
        {
            ApproveInvoiceResponse.Success => Ok(new ApproveInvoiceHttpResponse()),
            ApproveInvoiceResponse.Failure failure => BadRequest(new ProblemDetails
            {
                Detail = failure.ErrorMessage,
            }),
            _ => throw new UnreachableException(),
    ...
    [HttpPost("decline")]
    [AuthorizeFeature(Scope)]
    [EndpointName("DeclineInvoice")]
    [EndpointSummary("Decline an invoice")]
    [EndpointDescription("Decline an invoice. Only allowed for invoices with 'Created' status.")]
    [ProducesResponseType<DeclineInvoiceHttpResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status400BadRequest, MediaTypeNames.Application.ProblemJson)]
    public async Task<ActionResult<DeclineInvoiceHttpResponse>> Decline(
        [FromBody] DeclineInvoiceHttpRequest httpRequest,
        CancellationToken cancellationToken)
    {
        string userId = User.GetUserId();

        DeclineInvoiceResponse response = await _invoiceService.DeclineAsync(
            httpRequest.MapToApplication(userId),
            cancellationToken);

        return response switch
        {
            DeclineInvoiceResponse.Success => Ok(new DeclineInvoiceHttpResponse()),
            DeclineInvoiceResponse.Failure failure => BadRequest(new ProblemDetails
            {
                Detail = failure.ErrorMessage,
            }),
            _ => throw new UnreachableException(),
    ...
    [HttpPost("assign-accountant")]
    [AuthorizeFeature(Scope)]
    [EndpointName("AssignAccountant")]
    [EndpointSummary("Assign accountant to a corporate invoice")]
    [EndpointDescription("Assign accountant to a corporate invoice.")]
    [ProducesResponseType<AssignAccountantHttpResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status400BadRequest, MediaTypeNames.Application.ProblemJson)]
    public async Task<ActionResult<AssignAccountantHttpResponse>> AssignAccountant(
        [FromBody] AssignAccountantHttpRequest httpRequest,
        CancellationToken cancellationToken)
    {
        string userId = User.GetUserId();

        AssignAccountantResponse response = await _invoiceService.AssignAccountantAsync(
            httpRequest.MapToApplication(userId),
            cancellationToken);

        return response switch
        {
            AssignAccountantResponse.Success => Ok(new AssignAccountantHttpResponse()),
            AssignAccountantResponse.Failure failure => BadRequest(new ProblemDetails
            {
                Detail = failure.ErrorMessage,
            }),
            _ => throw new UnreachableException(),

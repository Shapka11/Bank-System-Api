...
internal sealed class UserService : IUserService
{
    ...
    public async Task<GetUsers.Response> GetAsyns(GetUsers.Request request, CancellationToken cancellationToken)
    {
        var query = UserQuery.Build(builder => builder
            .WithAuthorizationIds(request.AuthorizationIds)
            .WithIds(request.UserIds.Select(id => new UserId(id)))
            .WithPageSize(request.PageSize));
        User[] users = await _context.UsersRepository
            .QueryAsync(query, cancellationToken)
            .ToArrayAsync(cancellationToken);

        return new GetUsers.Response.Success(users.Select(u => u.MapToDto()).ToArray());
    }
}
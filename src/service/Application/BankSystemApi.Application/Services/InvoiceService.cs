...
internal sealed class InvoiceService : IInvoiceService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var senderAccountId = new AccountId(request.SenderAccountId);
        var receiverAccountId = new AccountId(request.ReceiverAccountId);

        Account? senderAccount = await _context.AccountsRepository
            .FindAccountByIdAsync(senderAccountId, cancellationToken);
        if (senderAccount is null)
        {
            _logger.LogWarning("Account with id {AccountId} not found.", senderAccountId.Value);
            return new CreateInvoice.Response.SenderAccountNotFound(senderAccountId.Value);
        }
        ...
        Account? receiverAccount = await _context.AccountsRepository
            .FindAccountByIdAsync(receiverAccountId, cancellationToken);
        if (receiverAccount is null)
        {
            _logger.LogWarning("Account with id {AccountId} not found.", receiverAccountId.Value);
            return new CreateInvoice.Response.ReceiverAccountNotFound(receiverAccountId.Value);
        }
        ...
        invoice = await _context.InvoicesRepository
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (user.Id != receiverAccount.UserId)
        {
            _logger.LogWarning(
                "Account {AccountId} does to belong to the user {UserId}",
        ...
        if (receiverAccount.IsCorporate() && invoice.State.State is not InvoiceStatus.Approved)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (receiverAccount.IsCorporate() && invoice.State.State != InvoiceStatus.Approved)
        {
            _logger.LogWarning("Invoice {InvoiceId} can't be revoked", invoiceId.Value);
            return new RevokeInvoice.Response.InvalidInvoiceState(
                "Invoice can't be revoked to the corporate account in not approved state");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (invoiceApproveResult is ApproveInvoiceResult.Failure failure)
        {
            ...
            return new ApproveInvoice.Response.InvalidInvoiceState(failure.ErrorMessage);
        }
        ...
        {
            _logger.LogWarning("Account with id {AccountId} not found.", invoice.SenderAccountId.Value);
            return new ApproveInvoice.Response.AccountNotFound(invoice.SenderAccountId.Value);
        }
        ...
        {
            _logger.LogWarning("Account with id {AccountId} not found.", invoice.ReceiverAccountId.Value);
            return new ApproveInvoice.Response.AccountNotFound(invoice.ReceiverAccountId.Value);
        }
        ...
        _logger.LogInformation("Invoice {InvoiceId} approved successfully", invoice.Id.Value);

        _metrics.IncInvoiceApproved();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (invoiceDeclineResult is DeclineInvoiceResult.Failure failure)
        {
            ...
            return new DeclineInvoice.Response.InvalidInvoiceState(failure.ErrorMessage);
        }
        ...
        {
            _logger.LogWarning("Account with id {AccountId} not found.", invoice.SenderAccountId.Value);
            return new DeclineInvoice.Response.AccountNotFound(invoice.SenderAccountId.Value);
        }
        ...
        _logger.LogInformation("Invoice {InvoiceId} declined successfully", invoice.Id.Value);

        _metrics.InvInvoiceDeclined();
        ...
        var query = InvoiceQuery.Build(builder =>
        {
            ...
            if (request.InvoiceType is InvoiceTypeDto.Incoming)
            {
                builder.WithReceiverAccountIds(accounts.Select(a => a.Id))
                    .WithSenderAccountIds(request.OtherUserAccountIds.Select(sai => new AccountId(sai)));
            }
            else
            {
                builder.WithSenderAccountIds(accounts.Select(a => a.Id))
                    .WithReceiverAccountIds(request.OtherUserAccountIds.Select(rai => new AccountId(rai)));
            }

            return builder;
        });

        Invoice[] invoices = await _context.InvoicesRepository

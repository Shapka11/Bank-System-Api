...
internal sealed class HistoryOperationRepository : IHistoryOperationRepository
{
    ...
    {
        const string sql = """
        INSERT INTO history_operations (account_id, occurred_at, payload)
        SELECT account_id, occurred_at, payload
        FROM unnest(:accountIds, :occurredAts, :payloads)
            AS source(account_id, occurred_at, payload)
        RETURNING history_operation_id, account_id, occurred_at, payload
        """;
    ...
    public async IAsyncEnumerable<HistoryOperation> QueryAsync(
        HistoryOperationQuery query,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        const string sql = """
        SELECT history_operation_id, account_id, occurred_at, payload
        FROM history_operations
        WHERE 
           (:cursor IS NULL OR history_operation_id > :cursor)
           AND (cardinality(:accountIds) = 0 OR account_id = ANY(:accountIds))
        ORDER BY history_operation_id
        LIMIT :page_size
        """;
    ...
    private HistoryOperation CreateHistoryOperation(DbDataReader reader)
    {
        ...
        var historyEntry = new HistoryOperationEntry(
            new HistoryOperationId(reader.GetInt64("history_operation_id")),
